...
def check_og1_flight_variables(ds: xr.Dataset, function_name: str):
    ...
    if function_name not in OG1_FLIGHT_MODEL_VARS:
        raise ValueError(f"Unknown function '{function_name}'. Available: {list(OG1_FLIGHT_MODEL_VARS.keys())}")
    ...
    if missing_optional:
        log_info(f"Optional variables missing for {function_name}: {list(missing_optional)}")
